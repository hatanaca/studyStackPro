...
function getCommentCount(): string {
  const stats = store.videoDetail?.statistics as Record<string, string> | undefined
      ...
      <template v-else>
              ...
              <div v-if="store.videoDetail" class="yt-detail">
                ...
                <div class="yt-detail__meta">
                  <span class="yt-detail__channel">
                    <span class="yt-detail__channel-avatar">{{
                      store.videoDetail.snippet.channelTitle.charAt(0).toUpperCase()
                    }}</span>
        ...
        <div v-if="store.results.length > 0" class="yt-toggle-wrap">
          <button class="yt-toggle-btn" @click="resultsVisible = !resultsVisible">
            ...
            <span class="yt-toggle-count"
              >{{ store.totalResults.toLocaleString('pt-BR') }} vídeos</span
            >
          </button>
        </div>
        ...
        <div v-show="resultsVisible" v-if="store.results.length > 0" class="yt-section">
          ...
          <TransitionGroup name="yt-card" tag="div" class="yt-grid">
            ...
            >
              ...
              <div class="yt-card__body">
                <div class="yt-card__avatar">
                  {{ item.snippet.channelTitle.charAt(0).toUpperCase() }}
                </div>
          ...
          <div class="yt-pagination">
            <button class="yt-pagination__btn" :disabled="!store.prevPageToken" @click="prevPage">
              <svg
                viewBox="0 0 24 24"
                width="16"
                height="16"
            ...
            <button class="yt-pagination__btn" :disabled="!store.nextPageToken" @click="nextPage">
              Próxima
              <svg
                viewBox="0 0 24 24"
                width="16"
...
.yt-search__btn:hover:not(:disabled) {
  background: var(--color-primary-hover);
}
.yt-search__btn:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
...
.yt-skeleton-line.w-75 {
  width: 75%;
}
.yt-skeleton-line.w-50 {
  width: 50%;
}
.yt-skeleton-line.w-33 {
  width: 33%;
}
@keyframes shimmer {
  0% {
...
.yt-slide-enter-active {
  transition: all 0.3s ease-out;
}
.yt-slide-leave-active {
