...
export interface TrackInfo {
  title: string; artist: string; thumbnail: string; videoId: string
}
...
function saveFavorites(list: FavoriteEntry[]) { try { localStorage.setItem(FAVORITES_KEY, JSON.stringify(list)) } catch {} }
...
export const usePlayerStore = defineStore('player', () => {
  ...
  // Persist watchers
  watch(isShuffled, v => { try { localStorage.setItem('studytrack_shuffle', String(v)) } catch {} })
  watch(repeatMode, v => { try { localStorage.setItem('studytrack_repeat', v) } catch {} })
  watch(volume, v => { try { localStorage.setItem('studytrack_volume', String(v)) } catch {} })

  const currentPlaylistId = computed(() => mode.value === 'playlists' || mode.value === 'favorites' ? selectedPlaylist.value?.id ?? null : null)
  const currentVideoId = computed(() => {
    if (mode.value === 'search' && searchResults.value.length > 0) {
      return searchResults.value[videoIndex.value]?.id?.videoId ?? searchResults.value[0]?.id?.videoId ?? null
    }
    return null
  })
  ...
  async function fetchPlaylists() {
    loadingPlaylists.value = true; playlistError.value = null
    try {
    ...
    } finally { loadingPlaylists.value = false; persist() }
  }
  function selectPlaylist(p: YouTubePlaylistItem) {
    mode.value = 'playlists'; selectedPlaylist.value = p; videoIndex.value = 0; isPlaying.value = true; isExpanded.value = true; persist()
  }
  ...
  function selectFavorite(entry: FavoriteEntry) {
    ...
    selectedPlaylist.value = { id: entry.playlistId, snippet: { title: entry.title, channelTitle: '', description: '', thumbnails: { medium: { url: entry.thumbnail }, high: { url: entry.thumbnail } }, publishedAt: '' } } as any
    videoIndex.value = 0; isPlaying.value = true; isExpanded.value = true; persist()
  }
  ...
  return {
    playlists, loadingPlaylists, playlistError,
    searchQuery, searchResults, searching, searchError, searchNextPageToken,
    favorites,
