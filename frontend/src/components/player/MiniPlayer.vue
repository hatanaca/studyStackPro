...
const playlistTitle = computed(
  () => player.selectedPlaylist?.snippet?.title ?? 'Selecionar playlist'
)
...
function clampPos() {
  ...
  const pw = containerRef.value?.offsetWidth ?? 280,
    ph = containerRef.value?.offsetHeight ?? 48
  pos.x = Math.max(0, Math.min(pos.x, w - pw))
  pos.y = Math.max(0, Math.min(pos.y, h - ph))
}
function loadPos() {
  try {
    ...
    if (r) {
      ...
      pos.x = p.x ?? 16
      pos.y = p.y ?? window.innerHeight - 60
    } else {
...
function savePos() {
  try {
    localStorage.setItem(STORAGE_POS_KEY, JSON.stringify({ x: pos.x, y: pos.y }))
  } catch {}
}
loadPos()
clampPos()
...
function onDragMove(e: MouseEvent | TouchEvent) {
  ...
  e.preventDefault()
  const p = 'touches' in e ? e.touches[0] : e
  pos.x = posStartX + (p.clientX - dragStartX)
  pos.y = posStartY + (p.clientY - dragStartY)
  clampPos()
}
function onDragEnd() {
  dragging = false
...
watch(hasGoogleAccount, (v) => {
  if (v) player.fetchPlaylists()
})

function handleSearch() {
  if (searchInput.value.trim()) {
    player.searchVideos(searchInput.value)
    player.switchMode('search')
  }
}
...
<template>
  <aside
    ...
    class="mini-player"
    :class="{
      'mini-player--expanded': player.isExpanded,
      'mini-player--dragging': dragging,
      'mini-player--hidden': uiStore.mobileSidebarOpen,
    }"
    :style="playerStyle"
  >
    <!-- Iframe invisível -->
    <div class="yt-player-wrapper">
      <YouTubeFrame
        ...
        :playlist-id="player.currentPlaylistId"
        :video-id="player.currentVideoId"
        :video-index="player.videoIndex"
        :is-playing="player.isPlaying"
        :is-shuffled="player.isShuffled"
    ...
    >
      <div
        ...
        :class="{ 'mini-player__mini-thumb--playing': player.isPlaying }"
        :style="{ backgroundImage: `url(${player.currentTrack.thumbnail})` }"
      />
      <span class="mini-player__title-label">{{
        player.currentTrack?.title || playlistTitle
      }}</span>
    ...
    >
      <div class="mini-player__handle">
        ...
        <span class="mini-player__header-title">{{
          player.mode === 'search'
            ? 'Busca'
            : player.mode === 'favorites'
              ? 'Favoritos'
              : 'Playlists'
        }}</span>
        <button
          v-if="player.mode === 'playlists' && player.selectedPlaylist"
          class="mini-player__btn-icon"
          :class="{
            'mini-player__btn-icon--active': player.isFavorite(player.selectedPlaylist.id),
          }"
          :title="player.isFavorite(player.selectedPlaylist.id) ? 'Salvo' : 'Salvar playlist'"
          @click.stop="player.addToFavorites()"
        >
          <svg
            v-if="player.isFavorite(player.selectedPlaylist.id)"
            xmlns="http://www.w3.org/2000/svg"
            width="14"
            height="14"
            viewBox="0 0 24 24"
      ...
      <div v-if="player.currentTrack" class="mini-player__track-info">
        ...
        <p v-if="player.currentTrack.artist" class="mini-player__track-artist">
      ...
      <div v-if="player.hasContent" class="mini-player__controls">
        ...
        <button
          ...
          :class="{ 'mini-player__ctrl-btn--active': player.isShuffled }"
        ...
        <button
          class="mini-player__ctrl-btn"
          :class="{ 'mini-player__ctrl-btn--active': player.repeatMode !== 'none' }"
          aria-label="Repetir"
          @click="player.cycleRepeat()"
        >
          <svg
            v-if="player.repeatMode !== 'single'"
            xmlns="http://www.w3.org/2000/svg"
            width="16"
            height="16"
            viewBox="0 0 24 24"
          ...
          >
            ...
            <polyline points="7 23 3 19 7 15" />
            <path d="M21 13v2a4 4 0 01-4 4H3" />
            <text x="11" y="16" font-size="8" font-weight="bold" fill="currentColor" stroke="none">
              1
            </text>
          </svg>
        </button>
      ...
      <div v-if="player.mode === 'playlists'" class="mini-player__section">
        <div v-if="!hasGoogleAccount" class="mini-player__msg">
          <a href="/api/v1/auth/google" class="mini-player__link"
            >Conectar Google para ver playlists</a
          >
        </div>
        <div v-else-if="player.loadingPlaylists" class="mini-player__msg">Carregando...</div>
        <div v-else-if="player.playlistError" class="mini-player__msg">
          {{ player.playlistError }}<br /><a href="/api/v1/auth/google" class="mini-player__link"
            >Reconectar Google</a
          >
        </div>
        <select
          v-else-if="player.playlists.length"
          class="mini-player__select"
          :value="player.currentPlaylistId"
          @mousedown.stop
          @change="
            player.selectPlaylist(
              player.playlists.find((p) => p.id === ($event.target as HTMLSelectElement).value)!
            )
          "
        >
          <option :value="null" disabled>Escolha uma playlist</option>
          <option v-for="pl in player.playlists" :key="pl.id" :value="pl.id">
            {{ pl.snippet.title }}
          </option>
        </select>
      ...
      <div v-if="player.mode === 'search'" class="mini-player__section">
        <div class="mini-player__search-bar">
          <input
            ...
            @mousedown.stop
          />
          <button class="mini-player__btn-set" :disabled="player.searching" @click="handleSearch">
            {{ player.searching ? '...' : 'Buscar' }}
          </button>
        </div>
        ...
        >
          <button
            ...
            @click="player.playSearchResult(i)"
          >
            <img
              v-if="item.snippet?.thumbnails?.medium?.url"
              :src="item.snippet.thumbnails.medium.url"
              class="mini-player__search-thumb"
              alt=""
            />
            <span class="mini-player__search-title">{{ item.snippet?.title }}</span>
          </button>
          <div v-if="player.searching && player.searchResults.length" class="mini-player__msg">
            Carregando mais...
          </div>
          <div
            v-if="!player.searchNextPageToken && player.searchResults.length"
            class="mini-player__msg"
          >
            Fim dos resultados
          </div>
        </div>
      ...
      <div v-if="player.mode === 'favorites'" class="mini-player__section">
        ...
        <div v-else class="mini-player__search-results">
          <div
            v-for="fav in player.favorites"
            :key="fav.playlistId"
            class="mini-player__search-item"
            role="button"
            tabindex="0"
            @click="player.selectFavorite(fav)"
            @keydown.enter="player.selectFavorite(fav)"
          >
            <img
              v-if="fav.thumbnail"
              :src="fav.thumbnail"
            ...
            <button
              class="mini-player__remove-fav"
              aria-label="Remover"
              @click.stop="player.removeFavorite(fav.playlistId)"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
...
.mini-player__collapse-bar:active {
  cursor: grabbing;
}
...
.mini-player__collapse-bar:hover .mini-player__collapse-grip {
  opacity: 0.8;
...
.mini-player__mini-thumb {
  ...
  flex-shrink: 0;
  cursor: pointer;
  box-shadow: 0 0 0 1.5px color-mix(in srgb, var(--color-primary) 30%, transparent);
  transition: box-shadow 0.2s;
}
.mini-player__mini-thumb--playing {
  animation: album-spin 20s linear infinite;
}
.mini-player__mini-thumb:hover {
  box-shadow: 0 0 0 1.5px var(--color-primary);
}
.mini-player__playing-dot {
  width: 5px;
  height: 5px;
  border-radius: 50%;
...
.mini-player__title-label:hover {
...
.mini-player__album-art {
  width: 100%;
  height: 100%;
  background-size: cover;
  background-position: center;
}
.mini-player__album-placeholder {
  width: 100%;
  height: 100%;
  display: flex;
  align-items: center;
...
.mini-player__progress::-webkit-slider-thumb:hover {
  transform: scale(1.2);
...
.mini-player__btn-icon:hover {
  background: color-mix(in srgb, var(--color-bg-soft) 80%, var(--color-text) 20%);
  color: var(--color-text);
...
.mini-player__btn-play:hover {
  transform: scale(1.1);
  box-shadow: 0 6px 24px color-mix(in srgb, var(--color-primary) 25%, transparent);
}
...
.mini-player__volume-slider::-webkit-slider-thumb:hover {
  background: var(--color-primary);
}
...
.mini-player__input:focus {
  border-color: var(--color-primary);
  box-shadow: 0 0 0 2px var(--color-primary-soft);
}
.mini-player__btn-set {
  padding: 6px 12px;
  border: 0;
  border-radius: var(--radius-md);
  background: linear-gradient(135deg, var(--color-primary), var(--color-primary-hover));
...
.mini-player__btn-set:hover {
  transform: translateY(-1px);
}
.mini-player__btn-set:disabled {
  opacity: 0.5;
  cursor: default;
  transform: none;
}
...
.mini-player__search-item:hover {
  background: color-mix(in srgb, var(--color-bg-soft) 80%, var(--color-text) 20%);
}
.mini-player__search-item--active {
  background: var(--color-primary-soft);
  color: var(--color-primary);
  box-shadow: inset 2px 0 0 var(--color-primary);
}
.mini-player__search-thumb {
  width: 44px;
  height: 30px;
  object-fit: cover;
  border-radius: var(--radius-md);
...
.mini-player__search-item--active .mini-player__remove-fav {
  opacity: 0.7;
}
.mini-player__remove-fav:hover {
  opacity: 1 !important;
...
@keyframes pulse-dot {
  ...
  50% {
    opacity: 0.4;
    box-shadow: 0 0 2px color-mix(in srgb, var(--color-primary) 25%, transparent);
  }
}
...
.mp-scale-enter-active {
  animation: mp-in 0.2s ease-out;
}
.mp-scale-leave-active {
  animation: mp-out 0.15s ease-in;
}
@keyframes mp-in {
  from {
    opacity: 0;
