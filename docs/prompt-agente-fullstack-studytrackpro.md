...
- **Frontend:** `src/api/` (client.ts, endpoints.ts, modules/*.api.ts), `src/stores/`, `src/router/`, `src/views/`, `src/components/ui` e `layout/`, `src/composables/`, `src/features/`, `src/types/`, `src/assets/styles/variables.css`.
- **Documentação:** `docs/SUMARIO_COMPLETO.md`, `README.md`, `AGENTS.md`.
...
- Tipagem TypeScript (props/emits e tipos de API em `types/`); chamadas apenas via módulos em `api/`.
- Estado global em Pinia (stores por domínio); design tokens e componentes base em `components/ui`.
...
- Referenciar `docs/SUMARIO_COMPLETO.md` para detalhes de fluxos, migrations, Docker e rate limiting.
...
- `docs/prompt-agente-frontend-studytrackpro.md` — prompt do agente frontend (referência de estilo, escopo e tecnologias modernas).
- `.cursor/rules/backend-studytrackpro.mdc` — regras do agente backend (complementar para tarefas de API, eventos e backend).
- `docs/prompt-agente-backend-studytrackpro.md` — prompt do agente backend (referência de arquitetura, checklist e tecnologias modernas).
