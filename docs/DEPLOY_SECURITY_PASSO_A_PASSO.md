...
O projeto já inclui vários deles em [docker/nginx/nginx.conf](../docker/nginx/nginx.conf). Em produção, descomente/ative o HSTS quando o SSL estiver em uso.
...
```bash
# Backend (raiz do projeto Laravel)
composer audit
