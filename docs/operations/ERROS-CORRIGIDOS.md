...
| **Arquivo(s)** | `docs/operations/ERROS-CORRIGIDOS.md`, `Makefile` (já contém `test-db-setup` e `test-back`). |
...
- **Contrato API**: Rotas em `backend/routes/api.php` (prefixo `v1`) e módulos em `frontend/src/api/` estão alinhados. Goals é apenas frontend (localStorage), conforme `docs/operations/GOALS-FRONTEND-ONLY.md`.
- **Migrations**: Sintaxe e ordem das migrations (transactional e analytics) estão coerentes.
- **CORS**: `backend/config/cors.php` usa `allowed_origins` vazio quando `CORS_ALLOWED_ORIGINS` não está definido — defina a variável para as origens permitidas (ver [ENV-VARS.md](ENV-VARS.md)).
