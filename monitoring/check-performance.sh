#!/usr/bin/env bash
# =============================================================================
# check-performance.sh — Verificação de Desempenho
#
# Executa: 1. PHPStan  2. Health check  3. Redis  4. Build frontend  5. Response time
#
# Uso: ./check-performance.sh [--cron] [--json] [--help]
# =============================================================================
...
check_frontend_build() {
    ...
    if [ -d "$FRONTEND_DIR/node_modules" ]; then
        ...
        else log_error "Build falhou"; grep -i 'error\|ERROR\|✖' "$bl" | head -3 | while IFS= read -r e; do log_error "  $e"; done; append_report "- **Build**: ❌"; r=1; fi
        rm -f "$bl"
...
check_response_time() {
    ...
    if [ "$code" = "200" ]; then
        log_ok "${dur}ms"; append_report "- **Response Time**: ✅ ${dur}ms"
        [ "$dur" -gt 1000 ] && log_warn "Alto (> 1s)"; return 0
